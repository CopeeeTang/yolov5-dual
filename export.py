...
def export_torchscript(model, im, im2,file, optimize, prefix=colorstr('TorchScript:')):
    # YOLOv5 TorchScript model export
    try:
        LOGGER.info(f'\n{prefix} starting export with torch {torch.__version__}...')
        f = file.with_suffix('.torchscript')
        print("执行此步1")
        ts = torch.jit.trace(model, [im,im2],strict=False)
        print("执行此步2")
...
def export_onnx(model, im, im2,file, opset, train, dynamic, simplify, prefix=colorstr('ONNX:')):
    # YOLOv5 ONNX export
    try:
        ...
        print("执行3")
        torch.onnx.export(model, (im,im2), f, verbose=False, opset_version=opset,
...
@torch.no_grad()
def run(data=ROOT / 'data/dual.yaml',  # 'dataset.yaml path'
    ...
    if jit:
        f[0] = export_torchscript(model, im,im2, file, optimize)
    if engine:  # TensorRT required before ONNX
        f[1] = export_engine(model, im, file, train, half, simplify, workspace, verbose)
    if onnx or xml:  # OpenVINO requires ONNX
        f[2] = export_onnx(model, im,im2, file, opset, train, dynamic, simplify)
    if xml:  # OpenVINO
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/dual.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default="C:/Users/33498/Desktop/Summer/result/double_result/weights/double.pt", help='model.pt path(s)')

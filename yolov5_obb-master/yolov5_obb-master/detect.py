...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    webcam = source.isnumeric() or source.endswith('.txt') or (is_url and not is_file)#数字字符串，以‘txt’结尾，是引用不是文件
    ...
    # Half (增加)如果设备不为CPU，且为pytorch框架使用半精度法
    ...
    if webcam:#判断是否为视频流
        ...
        dataset = LoadImages(source, img_size=imgsz, stride=stride, auto=pt)#抽帧
        bs = 1  # batch_size 帧数
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'E:/yolov5-master/yolov5_obb-master/Tianjin_0424/video/sources', help='file/dir/URL/glob, 0 for webcam')
